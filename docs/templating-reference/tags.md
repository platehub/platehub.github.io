...
- `pages`: an array of objects representing the pagination pages. All objects in `pages` have the attributes `url` (returning the url of a pagination page, e.g. /this-url?page=3), `index` (the pagination page's index number) and `is_link` (whether the pagination page is a link or not). `is_link` returns false at the current pagination page or if the page falls outside the `window_size` (in which case it's merely a '...').
